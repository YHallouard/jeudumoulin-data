...
class DQNTrainer:
    ...
    def train(
        ...
        batch_size = eval_frequency if eval_frequency else num_episodes

        for batch_start in range(0, num_episodes, batch_size):
            actual_batch = min(batch_size, num_episodes - batch_start)

            self.run_episode_batch(
                num_episodes=actual_batch,
                epsilon_start=max(epsilon_end, epsilon_start * (epsilon_decay**batch_start)),
                epsilon_end=epsilon_end,
                epsilon_decay=epsilon_decay,
                opponent=opponent,
                max_steps_per_episode=max_steps_per_episode,
            ...
            if eval_frequency and (batch_start + actual_batch) % eval_frequency == 0:
                eval_metrics = self.evaluate(num_games=eval_games, opponent=opponent, verbose=verbose)
                self.log_eval_metrics(eval_metrics, batch_start + actual_batch)
    ...
    def run_episode_batch(
        ...
        for episode in range(num_episodes):
            ...
            if save_frequency and (episode + 1) % save_frequency == 0 and save_folder:
                self.save_checkpoint(save_folder, total_episodes)
    ...
    def log_eval_metrics(self, eval_metrics: dict, step: int) -> None:
        self.mlflow_logger.log_metrics(
            {
        ...
        logger.info("evaluation_results_logged", step=step, win_rate=eval_metrics["win_rate"])

    def register_model(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            self.agent.save_pretrained(tmp_dir)
            self.mlflow_logger.register_model(tmp_dir, "jeudumoulin-dqn")

    def finish_mlflow(self) -> None:
        self.mlflow_logger.finish()

    def save_checkpoint(self, save_folder: Path, episode: int) -> None:
    ...
    def evaluate(self, num_games: int = 100, opponent: str = "random", verbose: bool = True) -> dict:
        ...
        if opponent == "random":
            ...
            raise ValueError(f"Unsupported opponent type: {opponent}")  # noqa: TRY003
    ...
    def _play_episode(self, epsilon: float, opponent: str, max_steps: int) -> tuple[float, int, str]:
        board = jdm_ru.PyBoard()
        agent_player = random.choice([1, -1])  # noqa: S311
        total_reward = 0.0
        step = 0
        ...
        while not board.is_terminal() and step < max_steps:
            ...
            if current_player == agent_player:
                ...
                if opponent == "random":
                    move = random.choice(legal_moves)  # noqa: S311
                elif opponent == "self":
                    state = board.to_embed()
                    action_idx = self.agent.select_action(state, legal_moves, epsilon=0.1)
                    move = legal_moves[action_idx]
                else:
                    raise ValueError(f"Unknown opponent type: {opponent}")  # noqa: TRY003
    ...
    def _log_progress(self, episode: int, total_episodes: int, epsilon: float) -> None:
        recent_rewards = self.episode_rewards[-10:]
        recent_losses = [l for l in self.episode_losses[-10:] if l > 0]  # noqa: E741
    ...
    def _log_final_statistics(self) -> None:
        ...
        if self.episode_losses:
            recent_losses = [l for l in self.episode_losses[-100:] if l > 0]  # noqa: E741
            if recent_losses:
...
@task(name="dqn-start-mlflow")
def dqn_start_mlflow_task(
    trainer: DQNTrainer,
    num_episodes: int,
    epsilon_start: float,
    epsilon_end: float,
...
@task(name="dqn-episode-batch")
def dqn_episode_batch_task(
    trainer: DQNTrainer,
    num_episodes: int,
    epsilon_start: float,
    epsilon_end: float,
...
@task(name="dqn-evaluate")
def dqn_evaluate_task(
    trainer: DQNTrainer,
    num_games: int = 50,
    opponent: str = "random",
    verbose: bool = True,
) -> dict:
    return trainer.evaluate(num_games=num_games, opponent=opponent, verbose=verbose)


@task(name="dqn-log-eval-metrics")
def dqn_log_eval_metrics_task(trainer: DQNTrainer, eval_metrics: dict, step: int) -> None:
    trainer.log_eval_metrics(eval_metrics, step)


@task(name="dqn-register-model")
def dqn_register_model_task(trainer: DQNTrainer) -> None:
    trainer.register_model()


@task(name="dqn-finish-mlflow")
def dqn_finish_mlflow_task(trainer: DQNTrainer) -> None:
